...
import entities.GrupoAtividade;

/**
 *
 * @author dgsantos
 */
public class GrupoGerenciar extends javax.swing.JInternalFrame {

    private Grupo grupo;
    private ArrayList<Grupo> listaGrupo;
    private ArrayList<GrupoAtividade> listaAtv;
    ...
    private void buscarGrupos() {
        ...
        listaGrupo = (ArrayList<Grupo>) daoService.select(temp);


        DefaultTableModel mdl = (DefaultTableModel) jTable1.getModel();
        mdl.setNumRows(0);

        for (Grupo rec : listaGrupo) {
    ...
    private void selecionarRecurso() {
        ...
        grupo = listaGrupo.get(row);


        txtId.setText("" + grupo.getId());
        txtNome1.setText(grupo.getNome());
        txtDescr.setText(grupo.getDescricao());
        if (grupo.isStatus()) {
    ...
    private void editarGrupo() {
        ...
        if (grupo != null) {
            ...
            GrupoCadastrar cadRec = new GrupoCadastrar(grupo);
    ...
    private void associarAtividade() {
        ...
        Principal.adicionarFrames(new GRUPO_ATIVIDADE(grupo));
    ...
    private void loadAtividade() {
        this.listaAtv = (ArrayList<GrupoAtividade>) this.grupo.query();
        DefaultTableModel mdl = (DefaultTableModel) jTable2.getModel();
        mdl.setNumRows(0);
        for (Iterator<GrupoAtividade> it = listaAtv.iterator(); it.hasNext();) {
            GrupoAtividade atv = it.next();
            Atividade get = atv.getAtividade();
            mdl.addRow(new Object[]{get.getId(), get.getNome(), atv.getTempo()});
        }
    }
}
